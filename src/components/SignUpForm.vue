      ...
      <v-card width="400px" class="mx-auto my-auto">
        <v-container fluid fill-height> 
          ...
          <v-row class="mt-1" align-content="center">          
            ...
            <v-col cols="12" md="4">
              <v-text-field v-bind:type="showPassword_1 ? 'text' : 'password'" @click:append="showPassword_1 = !showPassword_1" prepend-icon="mdi-lock" v-bind:append-icon="showPassword_1 ? 'mdi-eye' : 'mdi-eye-off'" v-model="password" label="パスワード"></v-text-field>
            </v-col>
            <v-col cols="15" md="4">
              <v-text-field v-bind:type="showPassword_2 ? 'text' : 'password'" @click:append="showPassword_2 = !showPassword_2" prepend-icon="mdi-lock" v-bind:append-icon="showPassword_2 ? 'mdi-eye' : 'mdi-eye-off'" v-model="re_password" label="パスワード(確認)"></v-text-field>
            </v-col>
            <p class="text-xl-body-2 red--text ml-11" v-if="matchpassword" >パスワードが違います</p>
          </v-row>
